...
import com.shreyas.imgurphotogallery.util.TestJsonUtils.getJsonAsString
...
abstract class MockServerBaseTest {
    ...
    fun provideTestImgurImageService(): IImgurPhotoService {
        return Retrofit.Builder().baseUrl(mockServer.url("/")).addConverterFactory(
            GsonConverterFactory.create()
        ).client(OkHttpClient.Builder().build()).build().create(IImgurPhotoService::class.java)
    }
}